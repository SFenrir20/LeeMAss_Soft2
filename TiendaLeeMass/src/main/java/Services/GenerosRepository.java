...
public class GenerosRepository implements IGeneros{
    ...
    public List<Generos> GetAll() {
         try {
            ...
            CallableStatement cs = Conexion.ObtenerConnection().prepareCall("{CALL ListarGen}");
    ...
    public List<Generos> getAllGeneros() {
        try {
             ...
             CallableStatement cs = Conexion.ObtenerConnection().prepareCall("{CALL ListarGeneros}");
             ResultSet rs = cs.executeQuery();
             while(rs.next()){
    ...
    public void addGeneros(Generos gen) {
            try {
            PreparedStatement ps = Conexion.ObtenerConnection().prepareCall("{CALL IngresarGeneros(?,?,?)}");
    ...
    public void removeGeneros(Generos gen) {
          try {
            PreparedStatement ps = Conexion.ObtenerConnection().prepareCall("{CALL EliminarGeneros(?)}");
            ps.setInt(1, gen.getId_Genero());
            ps.execute();
        } catch (Exception e) {
    ...
    public void updateGeneros(Generos gen) {
         try {
            PreparedStatement ps = Conexion.ObtenerConnection().prepareStatement("{CALL UpdateGeneros(?,?,?)}");
            ps.setInt(1, gen.getId_Genero());
            ps.setString(2, gen.getNombre_gen());
            ps.setString(3, gen.getDes_gen());
    ...
    public List<Generos> BuscarLibroXCodigo(int codigo) {
         try {
            List<Generos> lst = new ArrayList<>();
            CallableStatement cs = Conexion.ObtenerConnection().prepareCall("{CALL BuscarGeneros(?)}");
            cs.setInt(1, codigo);
            ResultSet rs = cs.executeQuery();
            while(rs.next()){
                Generos obj = new Generos();
    ...
    public int NewGenero() {
        int SgtNum = 0;
        try {
            CallableStatement cs = Conexion.ObtenerConnection().prepareCall("{CALL GetCorrelativoGen}");
            ResultSet rs = cs.executeQuery();
            while(rs.next()){
